...
reframe(falls=max(falls),
        eol=max(eol),
        elderly_emergency=max(elderly_emergency),
        frail=max(frail),
        amb_chronic=max(amb_chronic),
        amb_acute=max(amb_acute),
        amb_vacc_prev=max(amb_vacc_prev),
        expanded_eol=max(expanded_eol),
        age_range, sex, ethnic_group, imd_decile, icb_name_short)|>
  distinct(der_pseudo_nhs_number, der_financial_year, .keep_all = TRUE)cohort_overlap_data<-raw_cohort_overlap_data|>
...
save(cohort_overlap_data, file="Z:/Strategic Analytics/Projects 2024/NHSE Community Strategy/cohort_overlap_data.RData")
...
Overlap of the patients between cohorts is determined. The Venn diagrams are coloured so that the largest groups/overlap is shown in red and the smallest groups/overlap is shown in blue. The Upset plots show only the largest groups/overlaps.
...
The most common cohort for those who are White British was the elderly emergency cohort, followed by those in both the elderly emergency and frail cohorts. For all other ethnicities the most common cohort was the acute ambulatory care sensitive conditions cohort. The second most common cohort in all those who were not white was the chronic ambulatory care sensitive conditions cohort.
